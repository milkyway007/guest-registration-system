const monthNames = [
  ...
  const initializeFields = (participant) => {
    ...
    if (!participant.participantsCount) {
      ...
      console.log(personPaymentMethodSelect.value)
...
*/
  const load = () => {
    ...
    for (const person of participants) {
      console.log(person);
      var liElement = person.render();
      console.log(liElement);
  
      const startDeleteParticipantButton = liElement.querySelector(
        ".start-delete-participant-button"
      );
      const startModifyParticipantButton = liElement.querySelector(
